...
const finishCityCombat = function(battle, victor) {
    ...
    case 2:
      ...
      if (victor == 'attacker') {
        ...
        if (battle.defendCountry != null) {
            // the defender is a country
            defenderName = countries[battle.defendCountry].name;
            map2Cities[battle.cityID].occupiedBy = defenderName;
            map2Cities[battle.cityID].occupierID = battle.defendCountry;
        }  else {
      ...
      // now that the city is occupied, check to see if other cities in this province are occupied and in that case
      // a timer must start for the province to secede, or police changes to be made, or autonomy to increase,
      // or ideological or ethnic changes to be made, or any other provincial changes.
  ...
  cityBattles[battle.cityID].finished = true;
  // finally conclude the city battle
}
